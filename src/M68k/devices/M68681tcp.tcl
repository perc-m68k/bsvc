...
proc DeviceSetupGetValues {} {
  ...
  set tcp_port [.device.tcp_port.entry get]

  set result "BaseAddress = $base TcpPort = $tcp_port OffsetToFirstRegister = $offset_to_first OffsetBetweenRegisters = $offset_between InterruptLevel = $interrupt_level PortAStandardInputOutputFlag = $port_a_std PortBStandardInputOutputFlag = $port_b_std PortACommand = $port_a_command PortBCommand = $port_b_command"
...
proc DeviceSetup {} {
  ...
  toplevel .device
  wm title .device "M68681 TCP Setup"
  wm iconname .device "M68681 TCP Setup"

  message .device.message \
    -text "Please enter appropriate values for the M68681's & TCP's options" \
  ...
  frame .device.base -relief groove -borderwidth 2
    ...
    bind .device.base.entry <Return> { focus .device.tcp_port.entry }
    pack .device.base.label -side left
    pack .device.base.entry -side left -fill x -expand 1 -pady 2 -padx 2

  frame .device.tcp_port -relief groove -borderwidth 2
    label .device.tcp_port.label -text "TCP port:"
    entry .device.tcp_port.entry -width 10 -relief sunken
    bind .device.tcp_port.entry <Return> { focus .device.portA.entry }
    pack .device.tcp_port.label -side left
    pack .device.tcp_port.entry -side left -fill x -expand 1 -pady 2 -padx 2
  ...
  pack .device.tcp_port -side top -fill x -pady 4 -padx 4
  ...
  # Set some defaults
  .device.tcp_port.entry set 23230
