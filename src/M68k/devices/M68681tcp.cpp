//
// The Motorola 68681 DUART WITH TCP ASYNC
//
...
long M68681_tcp::baudrate_table[32] = {
...
// The Constructor
M68681_tcp::M68681_tcp(const std::string &args, BasicCPU &cpu)
  ...
  in >> keyword >> equals >> std::hex >> port;
  if ((!in) || (keyword != "TcpPort") || (equals != "=")) {
    ErrorMessage("Invalid initialization arguments!");
    return;
  }
...
// Startup a process
bool M68681_tcp::StartPortCommand(const std::string &command, bool std_flag,
...
bool M68681_tcp::CheckMapped(Address address) const {
...
Byte M68681_tcp::Peek(Address addr) {
...
void M68681_tcp::Poke(Address addr, Byte c) {
  addr -= (base_address + offset_to_first_register);
  if (addr == 0) {  // Mode Register A
    if (mr1a_pointer) {
      mr1a_pointer = 0;
...
void M68681_tcp::EventCallback(int type, void *) {
  SetInterruptStatusRegister();
...
void M68681_tcp::SetInterruptStatusRegister() {
  ISR = 0;
...
int M68681_tcp::InterruptAcknowledge(unsigned int) {
